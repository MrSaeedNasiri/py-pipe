...
def local_commit():
    return subprocess.run(shlex.split("git rev-parse main"), cwd=project_dir,capture_output=True).stdout.decode("utf-8").rstrip()

def remote_commit():
    ps = subprocess.Popen(["git","ls-remote",project_repo,"refs/heads/main"], cwd=project_dir,stdout=subprocess.PIPE)
    output = subprocess.check_output(["cut","-f","1"],stdin=ps.stdout).decode("utf-8").rstrip()
    ps.wait()
    return output
...
while(True):
    ...
    if(local_commit() != remote_commit()):
        ...
        print('installed')
        subprocess.run(shlex.split("yarn build:prod"), cwd=project_dir,capture_output=True)
        print('builded')
